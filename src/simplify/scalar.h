...
#           define SCALAR_FROM_STRING(X, Z) mpfr_init(Z), mpfr_set_str((Z), (X), 10, MPFR_RNDN)
#           define SCALAR_POW(X, Y, Z)      mpfr_pow((Z), (X), (Y), MPFR_RNDN)
#           define SCALAR_SET(X, Z)         mpfr_init(Z), mpfr_set((Z), (X), MPFR_RNDN)
#           define SCALAR_DEFINE(X)         scalar_t X; mpfr_init(X)
...
#           define SCALAR_FROM_STRING(X, Z) mpf_init(Z), mpf_set_str((Z), (X), 10)
#           define SCALAR_POW(X, Y, Z)      mpf_pow_ui((Z), (X), mpf_get_ui(Y))
#           define SCALAR_SET(X, Z)         mpf_init(Z), mpf_set((Z), (X))
#           define SCALAR_DEFINE(X)         scalar_t X; mpf_init(X)
